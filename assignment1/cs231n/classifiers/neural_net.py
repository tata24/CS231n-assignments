...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        scores_yi = scores[range(N), list(y)]
        loss = np.sum(-scores_yi + np.log(np.sum(np.exp(scores), axis=1))) / N
        ...
        softmax_out[range(N), list(y)] -= 1
        dscores = softmax_out / N
        grads['W2'] = h_Relu.T.dot(dscores) + 2 * reg * W2
